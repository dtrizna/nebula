...
if __name__ == "__main__":
    yHashesFiles = rf"{SCRIPT_PATH}\..\data_filtered\speakeasy_trainset_WithAPIargs\speakeasy_yHashes.json"
    ...
    for i, sampleHash in tqdm(enumerate(yHahses)):
        # skip if done
        if [x for x in os.listdir(outPath) if x.endswith(".npy")]:
            arrName = os.listdir(outPath)[0]
            idx = int(arrName.split("_")[-1].split(".")[0])
            if i < idx:
                continue
