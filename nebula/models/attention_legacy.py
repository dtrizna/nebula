...
class TransformerEncoderChunks(nn.Module):
    """
    Slices global attention to multiple separate chunks. 'Chunk' term borrowed from longformer: 
    ...
    def __init__(self,
        ...
        self.mean_over_sequence = mean_over_sequence
    ...
    def core(self, src: Tensor, src_mask: Optional[Tensor] = None) -> Tensor:
        ...
        x = torch.mean(x, dim=1) if self.mean_over_sequence else x.view(x.size(0), -1)
        x = self.ffnn(x)
        return x
...
class TransformerEncoderChunksLM(TransformerEncoderChunks):
    def __init__(self, pretrain_layers: list = [1024], *args, **kwargs):
        ...
        # NOTE: bias removed the same as in https://github.com/karpathy/nanoGPT/blob/master/model.py#L133
        if self.pretrain_layers:
            self.pretrain_layers.append(nn.Linear(pretrain_layers[-1], self.vocab_size, bias=False))
        else:
            self.pretrain_layers.append(nn.Linear(self.hiddenNeurons[-1], self.vocab_size, bias=False))
