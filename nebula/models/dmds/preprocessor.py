...
class DMDSPreprocessor(object):
    ...
    def pad_sequence(self, seq_len=None):
        if seq_len:
            assert isinstance(seq_len, int), "seq_len must be integer!"
            self.seq_len = seq_len
        if self.x.shape[0] >= self.seq_len:
            return self.x[:self.seq_len, :]
        else:
            padded = np.pad(
                self.x, 
                ((0, self.seq_len - self.x.shape[0]), (0,0)),
                mode='constant', 
                constant_values=0
            )
            return padded
    ...
    def parse(self, api_name, args):
        ...
        api_ints, api_strings = [], []
        for arg_value in args:
            if isinstance(arg_value, (list, dict, tuple)):
                continue
            if isinstance(arg_value, (int, float)):
                api_ints.append(np.log(np.abs(arg_value) + 1))
            else:
                if arg_value[:2] == '0x':
                    continue
                api_strings.append(arg_value)

        # dirty temp hack
        api_int_dict = dict(zip(['']*len(api_ints), api_ints))
        api_str_dict = dict(zip(['']*len(api_strings), api_strings))
        try:
            ...
            logging.error(f"Task error: {api_name} | {args}\n\tException: {e}")
            return None