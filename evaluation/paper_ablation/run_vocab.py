...
if __name__ == "__main__":
    if INFOLDER:
        ...
        out_folder_root = f"out_vocab_{int(time.time())}"
    ...
    for VOCAB in [int(x) for x in [5e3, 1e4, 3e4, 5e4, 7e4]]:
        datafolders[VOCAB] = os.path.join(out_folder_root, f"nebula_vocab_{VOCAB}_seqlen_{SEQ_LEN}")
        ...
        _, y_train, y_paths_train, = preprocess_nebula_speakeasy(
            ...
            vocab_size=VOCAB,
            seq_len=SEQ_LEN,
            outfolder=datafolders[VOCAB],
            tokenizer_type=TOKENIZER_TYPE
        )
        if TOKENIZER_TYPE == "bpe":
            tokenizer_model = os.path.join(datafolders[VOCAB], f"tokenizer_{VOCAB}.model")
        else:
            tokenizer_model = os.path.join(datafolders[VOCAB], f"tokenizer_{VOCAB}_vocab.json")
        ...
        # ============= DEFINE MODEL =============
        with open(os.path.join(datafolders[VOCAB], f"tokenizer_{VOCAB}_vocab.json")) as f:
            nebula_vocab = json.load(f)
        models[VOCAB]['class'] = TransformerEncoderChunks
        models[VOCAB]['config'] = {
        ...
        # ============= TRAINING LOOP ============= 
        logging.warning(f" [!!!] Starting CV over {VOCAB}!")
        
        suffix = LIMIT if LIMIT else "full"
        x_train = np.load(os.path.join(datafolders[VOCAB], f"x_train_{suffix}.npy"))
        y_train = np.load(os.path.join(datafolders[VOCAB], f"y_train_{suffix}.npy"))
        
        model_class = models[VOCAB]['class']
        model_config = models[VOCAB]['config']
        ...
        cv_outfolder = os.path.join(out_folder_root, f"cv_{VOCAB}_lim{LIMIT}_r{RANDOM_SEED}_t{TIME_BUDGET}")
        cv = CrossValidation(
            model_trainer_class,
            model_trainer_config,
            model_class,
        ...
        cv.dump_metrics(prefix=f"{VOCAB}")
        cv.log_avg_metrics()
