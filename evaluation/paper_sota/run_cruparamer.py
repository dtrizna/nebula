...
SEQ_LEN = 512
# if 512, takes ~100h to process all train data
CRUPARAMER_SEQ_LEN = 64 
parse_test = False
...
if __name__ == "__main__":
    ...
    if INFOLDER and os.path.exists(os.path.join(out_folder_root, "reports.json")):
        y = np.load(os.path.join(out_folder_root, "y.npy"))
        reports = json.load(open(os.path.join(out_folder_root, "reports.json")))
        reports_apis_only = json.load(open(os.path.join(out_folder_root, "reports_apis_only.json")))
        dmds_reports = json.load(open(os.path.join(out_folder_root, "dmds_reports.json")))
    else:
        ...
        json.dump(reports, open(os.path.join(out_folder_root, "reports.json"), "w"))
        json.dump(reports_apis_only, open(os.path.join(out_folder_root, "reports_apis_only.json"), "w"))
        json.dump(dmds_reports, open(os.path.join(out_folder_root, "dmds_reports.json"), "w"))
        np.save(os.path.join(out_folder_root, "y.npy"), y)
    

    if INFOLDER and parse_test and os.path.exists(os.path.join(out_folder_root, "reports_test.json")):
        y_test = np.load(os.path.join(out_folder_root, "y_test.npy"))
        reports_test = json.load(open(os.path.join(out_folder_root, "reports_test.json")))
        reports_apis_only_test = json.load(open(os.path.join(out_folder_root, "reports_apis_only_test.json")))
        dmds_reports_test = json.load(open(os.path.join(out_folder_root, "dmds_reports_test.json")))
    else:
        if parse_test:
            ...
            json.dump(reports_test, open(os.path.join(out_folder_root, "reports_test.json"), "w"))
            json.dump(reports_apis_only_test, open(os.path.join(out_folder_root, "reports_apis_only_test.json"), "w"))
            json.dump(dmds_reports_test, open(os.path.join(out_folder_root, "dmds_reports_test.json"), "w"))
            np.save(os.path.join(out_folder_root, "y_test.npy"), y_test)
