...
EMULATION_TRAINSET_PATH = os.path.join(
    REPO_ROOT, "..", "..", "Data", "Nebula_Kaggle", "windows_emulation_trainset"
)
EMULATION_TESTSET_PATH = os.path.join(
    REPO_ROOT, "..", "..", "Data", "Nebula_Kaggle", "windows_emulation_testset"
)
...
def main(limit=None):
    ...
    # === PARSING PE FILES ===
    subFoldersTrain = [
        os.path.join(EMULATION_TRAINSET_PATH, x)
        for x in os.listdir(EMULATION_TRAINSET_PATH)
        if x.startswith("report_")
    ]
    eventsTrain, yTrain, yHashesTrain = readAndFilterFolders(
        subFoldersTrain, extractor.filter_and_normalize_report, limit=limit
    )

    subFoldersTest = [
        os.path.join(EMULATION_TESTSET_PATH, x)
        for x in os.listdir(EMULATION_TESTSET_PATH)
        if x.startswith("report_")
    ]
    eventsTest, yTest, yHashesTest = readAndFilterFolders(
        subFoldersTest, extractor.filter_and_normalize_report, limit=limit
    )
    ...
    for vocab_size in VOCABS.keys():
        ...
        TRAIN_OUT_FOLDER = os.path.join(
            REPO_ROOT, "data", "data_filtered", f"speakeasy_trainset{OUTFOLDER_SUFFIX}"
        )
        os.makedirs(TRAIN_OUT_FOLDER, exist_ok=True)

        TEST_OUT_FOLDER = os.path.join(
            REPO_ROOT, "data", "data_filtered", f"speakeasy_testset{OUTFOLDER_SUFFIX}"
        )
        ...
        np.save(
            os.path.join(TRAIN_OUT_FOLDER, f"speakeasy_y.npy"),
            np.array(yTrain, dtype=np.int8),
        )
        np.save(
            os.path.join(TEST_OUT_FOLDER, f"speakeasy_y.npy"),
            np.array(yTest, dtype=np.int8),
        )
        ...
        for maxSeqLen in MAX_SEQ_LENGTHS:
            ...
            if os.path.exists(
                os.path.join(TRAIN_OUT_FOLDER, f"{filePrefix}_x.npy")
            ) and os.path.exists(os.path.join(TEST_OUT_FOLDER, f"{filePrefix}_x.npy")):
                logging.warning(
                    f" [!] Skipping {filePrefix} because files already exist"
                )
                continue
            ...
            logging.warning(
                f"Saving files with prefix: {filePrefix}_maxlen_{maxSeqLen}"
            )
            np.save(
                os.path.join(
                    TRAIN_OUT_FOLDER, f"{filePrefix}_maxlen_{maxSeqLen}_x.npy"
                ),
                eventsEncodedTrain,
            )
            np.save(
                os.path.join(TEST_OUT_FOLDER, f"{filePrefix}_maxlen_{maxSeqLen}_x.npy"),
                eventsEncodedTest,
            )
...
def readAndFilterFolders(subFolders, parserFunction, limit=None):
    ...
    for subFolder in subFolders:
        ...
        files = [
            ...
            for x in os.listdir(subFolder)[:limit]
            if x.endswith(".json")
        ]
